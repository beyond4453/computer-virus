...
// _addr_GetModuleHandleA是存放VirtualProtect()的全局变量
...
DWORD GetIAFromImportTable(DWORD dwBase, LPCSTR lpszFuncName)
{
	...
	__asm{
		lea ecx, dword ptr[old] 
		push ecx
		push PAGE_READWRITE
		push 4
		push pdwAddr
		lea  ecx, [ebx + _addr_VirtualProtect]
        call dword ptr [ecx]
	}
	*pdwAddr = (DWORD)MyMessageBoxA;
    return (DWORD)pdwAddr;
...
{
    int off; 
    DWORD func_addr, func1_addr, func2_addr;
    HMODULE hModule;
    __asm {
	...
	func1_addr = (DWORD)GetProcAddress(hModule, "VirtualProtect");
	func2_addr = (DWORD)GetProcAddress(hModule, "MessageBoxA");	
    ...
    *(PDWORD)((PBYTE)_addr_GetModuleHandleA + off) = func_addr;
	...
	if(func1_addr == 0) {
		printf("[E]: VirtualProtect not found. \n");
		return 0;
	}
	 off = (DWORD)pCode - (DWORD)pOrignCode;
    *(PDWORD)((PBYTE)_addr_VirtualProtect + off) = func1_addr;
	...
	if(func2_addr == 0) {
	 ...
	 off = (DWORD)pCode - (DWORD)pOrignCode;
    *(PDWORD)((PBYTE)_addr_MessageBoxA + off) = func2_addr;
